...
import { jobsService, JobListing, JobListingInput } from "@/services/jobs";
import { applicationsService, JobApplication, JobApplicationInput } from "@/services/applications";
...
export function useJobListings() {
  ...
  const fetchJobs = useCallback(async (filters?: {
    ...
    try {
      const jobListings = await jobsService.getJobListings(filters);
      setJobs(jobListings);
      return jobListings;
    } catch (err: any) {
  ...
  const updateJob = useCallback(async (jobId: string, updates: Partial<JobListingInput>) => {
    ...
    try {
      const updatedJob = await jobsService.updateJobListing(jobId, updates);
  ...
  const searchJobs = useCallback(async (searchTerm: string) => {
    ...
    try {
      const results = await jobsService.searchJobListings(searchTerm);
      setJobs(results);
      return results;
    } catch (err: any) {
